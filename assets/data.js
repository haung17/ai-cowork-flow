...
AppData.flowcharts = {
  main: {
    ...
    nodes: [
      ...
      { id: 'I', role: ROLE.ENG,     type: TYPE.HUMAN,    title: '開發實作',               bullets: ['功能開發','Code Review','PR 提交'],                           x: 45, y: 50 },
  ...
  preDev: {
    ...
    nodes: [
      ...
      { id:'L', role:ROLE.PM,     type:TYPE.HUMAN,    title:'整合排程 / 分派任務 / 里程碑', bullets:['Sprint 計畫','任務指派','里程碑確認'],                   x:45, y:65 },
    ],
    edges: [
      {from:'A',to:'B'},{from:'B',to:'C'},{from:'C',to:'D'},
      {from:'D',to:'E'},{from:'E',to:'F'},{from:'F',to:'G'},
      {from:'F',to:'J'},{from:'G',to:'H'},{from:'H',to:'I'},
      {from:'J',to:'K'},{from:'I',to:'L'},{from:'K',to:'L'},
    ]
  },
  midDev: {
    ...
    nodes: [
      ...
      { id:'B', role:ROLE.ENG,    type:TYPE.HUMAN,    title:'實作功能',                  bullets:['功能開發','Code Review','技術決策'],                        x:45, y:9  },
      { id:'C', role:ROLE.ENG,    type:TYPE.COWORK,   title:'PR / 文件 / 測試碼草稿',    bullets:['PR 摘要','文件草稿','測試 skeleton'],                       x:70, y:9  },
      { id:'D', role:ROLE.SYSTEM, type:TYPE.SYSTEM,   title:'GitHub Actions 自動跑測試', bullets:['Smoke test','E2E','Regression'],                           x:70, y:16 },
      ...
      { id:'G', role:ROLE.PM,     type:TYPE.COWORK,   title:'每日進度摘要 / 阻塞提醒',   bullets:['今日完成','進行中','阻塞項目','需 PM 協助'],                x:20, y:30 },
      { id:'H', role:ROLE.PM,     type:TYPE.HUMAN,    title:'排除阻塞 / 協調優先序',     bullets:['阻塞排除','資源協調','客戶溝通'],                           x:20, y:37 },
    ...
    edges: [
      {from:'A',to:'B'},{from:'B',to:'C'},{from:'C',to:'D'},
      {from:'D',to:'E'},{from:'B',to:'F'},{from:'E',to:'F'},
      {from:'F',to:'G'},{from:'G',to:'H'},{from:'H',to:'I'},
  ...
  postDev: {
    ...
    edges: [
      ...
      {from:'H',to:'J',label:'否'},{from:'J',to:'K'},{from:'K',to:'L'},
      {from:'L',to:'M'},{from:'M',to:'N'},{from:'N',to:'O'},
    ]
  }
};
...
AppData.slides = [
  ...
  },
  {
    ...
    rows: [
      ['開發前', '需求訪談、範圍確認、排程', '需求可測性檢查、測試規劃', '技術評估、工時估算'],
      ['開發中', '進度追蹤、需求變更管控', '功能測試、Bug 回報、回歸', '功能實作、Code Review'],
      ['開發後', 'UAT 安排、結案、文件', '完整回歸、放行評估', 'Bug 修復、上線支援'],
    ]
  },
  ...
  { id: 'flow-postdev', type: 'flowchart', chartId: 'postDev', title: '開發後流程圖' },
  {
    ...
    cols: [
      { label: '第一層：可全自動', color: 'green', items: ['自動跑測試', '自動產進度摘要', '自動抓 issue 狀態', '自動產 Release Note 草稿'] },
  ...
  },
  {
    ...
    rows: [
      ...
      { phase: '開發中', auto: '每日進度摘要、阻塞任務提醒', draft: '規格差異比對、客戶回報分類、Demo 腳本', human: '範圍變更接受與否、與客戶談加價' },
      { phase: '開發後', auto: 'Release Note、結案清單', draft: 'UAT 問題整理、交接文件草稿', human: '是否放行上線、客戶簽核' },
    ]
  },
  {
    id: 'qa-cowork',
    ...
    rows: [
      { phase: '開發前', auto: '需求 × 測試對照表', draft: '測試案例初稿、邊界案例、測試資料草稿', human: '測試策略、高風險判定' },
      { phase: '開發中', auto: 'Smoke / Regression 自動執行', draft: '回歸測試建議、Bug ticket 草稿、失敗原因初判', human: 'Bug severity、放行建議' },
      { phase: '開發後', auto: '—', draft: 'UAT 問題分類、測試報告、已知問題清單', human: '是否允許上線' },
    ]
  },
  {
    id: 'mvp',
